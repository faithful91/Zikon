...
$req = $bdd->prepare('SELECT titres.id,titres.nom,albums.cover,artistes.nom as anom FROM titres_albums,albums,titres,artistes_titres,artistes WHERE
							 titres_albums.titre = titres.id and titres_albums.album = albums.id and artistes_titres.artiste = artistes.id and
           					 artistes_titres.titre = titres.id and artistes.id = ?
							 order by titres.nom');
$req->execute(array($_POST['artiste']));
